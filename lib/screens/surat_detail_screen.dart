...
class _SuratDetailScreenState extends State<SuratDetailScreen> {
  ...
  Widget _buildAlurDisposisiTimeline() {
    ...
    // STEP 3: Kabag / Sespri (True ONLY when Kabag has forwarded/disposed AND completed the incoming disposition)
    final bool hasForwardedFromKabag = list.any((d) =>
    ...
    final bool isKaroToKabagSelesai = list.any((d) {
      final isToKabag = d.kepadaJabatan.toLowerCase().contains('bagian') ||
          d.kepadaJabatan.toLowerCase().contains('kabag') ||
          d.kepadaJabatan.toLowerCase().contains('sespri') ||
          d.kepadaRole.toLowerCase().contains('kabag');
      final isSelesai = d.statusDisposisi.toLowerCase() == 'selesai';
      return isToKabag && isSelesai;
    });

    final bool isKabagSespriDone = hasForwardedFromKabag && isKaroToKabagSelesai;
    ...
    int currentStep = 1;
    final String globalStatus = _arsip.statusGlobal.toLowerCase().trim();
    if (globalStatus == 'menunggu_karo') {
      currentStep = 2;
    } else if (globalStatus == 'menunggu_kabag') {
      currentStep = 3;
    } else if (globalStatus == 'menunggu_katim') {
      currentStep = 4;
    } else if (globalStatus == 'selesai') {
      currentStep = 4;
    }
          ...
          Wrap(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  _arsip.statusGlobal.toLowerCase().trim() == 'selesai'
                      ? 'Selesai'
      ...
      child: Column(
        ...
        children: [
          ...
          ...historyList.asMap().entries.map((entry) {
            ...
            final isItemToKatim = item.kepadaJabatan.toLowerCase().contains('tim kerja') ||
                item.kepadaJabatan.toLowerCase().contains('katim') ||
                item.kepadaRole.toLowerCase().contains('katim');

            final isItemToKabag = item.kepadaJabatan.toLowerCase().contains('bagian') ||
                item.kepadaJabatan.toLowerCase().contains('kabag') ||
                item.kepadaJabatan.toLowerCase().contains('sespri') ||
                item.kepadaRole.toLowerCase().contains('kabag');

            final bool isAuthorizedToComplete = isItemToKatim
                ? PermissionService.isKatim
                : (isItemToKabag ? PermissionService.isKabag : false);
            ...
            Color circleColor;
            switch (item.statusDisposisi.toLowerCase().trim()) {
              case 'selesai':
              case 'diselesaikan':
